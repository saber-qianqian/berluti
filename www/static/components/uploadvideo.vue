<style lang="less">
	.upload-audio-box{
		...
		.ntBdaolpu{ .left; }
		.audio_show{
			.left;.w(500px);.ml(20px);

			.upload_audio{ margin: 10px; }
		}
		.upload_audio .btn-group div.btn-default{
			padding: 9px 12px 8px;
		}
		.upload_audio .btn-group .btn{
			.h(34px);box-sizing: border-box;
		}
	}
</style>
			...
			initAudio: function(){
				$('audio[controls]').before(function() {
					...
					var load_error = function() {
						...
						$(player_box).find('.glyphicon-refresh').text('失败');
						$(player_box).find('.glyphicon-refresh').parent().attr('title', '音频加载错误。');
					...
					var addTime = function() {
						...
						time.showtime = function() {
							...
							if (!song.paused) {
								...
								$(time).attr({
									'title': length_title + (time.timesplit(song.duration))
								});
							} else {
								$(time).html(time.timesplit(song.duration));
								$(time).attr({
									'title': position_title + (time.timesplit(song.currentTime))
								});
							}
					...
					var addVolume = function() {
						...
						$(vol_wrapper).addClass('btn  btn-default  row col-sm-4  hidden-xs');
