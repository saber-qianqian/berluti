...
var vm = new Vue({
	...
	data: {
		...
		moreParams: ['lesson_id=' + (params.lesson_id || '')],
	...
	methods: {
		...
		openCreateUrl: function(id){
			var url = '/manage/chapter/create/main?'
			var _url_params = []

			if(params.lesson_id) _url_params.push('lesson_id=' + params.lesson_id)
			if(params.course_id) _url_params.push('course_id=' + params.course_id)
			if(id) _url_params.push('id=' + id)

			window.location.href = url + _url_params.join('&')
		},
